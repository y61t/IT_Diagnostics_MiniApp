...
// Отправка формы с валидацией и chat_id
contactForm.addEventListener("submit", async (e) => {
  ...
  const data = { name, email, telegram, scenario: selectedScenario, chat_id: userId };
  ...
  try {
    ...
    if (response.ok && result.status === "ok") {
      showSuccess("Спасибо! Чек-лист уже отправлен вам в Telegram. Наш архитектор пришлёт дорожную карту.");
      contactForm.reset();
